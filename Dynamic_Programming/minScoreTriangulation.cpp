#include<bits/stdc++.h>
using namespace std;

class Solution {
private:
    int recursive(vector<int>& v, int i, int j) {
        if (i + 1 == j)
            return 0; // Only two vertex

        int ans = INT_MAX;
        for (int k = i + 1; k < j; k++) {
            int temp =
                v[i] * v[j] * v[k] + recursive(v, i, k) + recursive(v, k, j);
            ans = min(ans, temp);
        }

        return ans;
    }

    int memoization(vector<int>& v, int i, int j, vector<vector<int>>& dp) {
        if (i + 1 == j)
            return 0; // Only two vertex

        if (dp[i][j] != -1)
            return dp[i][j];

        int ans = INT_MAX;
        for (int k = i + 1; k < j; k++) {
            int temp = v[i] * v[j] * v[k] + memoization(v, i, k, dp) +
                       memoization(v, k, j, dp);
            ans = min(ans, temp);
        }

        dp[i][j] = ans;
        return dp[i][j];
    }

    int tabulation(vector<int>& v){
        int n = v.size();
        vector<vector<int>> dp(n, vector<int>(n, 0));

        for(int i = n-1; i >= 0; i--){
            for(int j = i+2; j < n; j++){
                int ans = INT_MAX;
                for (int k = i + 1; k < j; k++) {
                    int temp = v[i] * v[j] * v[k] + dp[i][k] + dp[k][j];
                    ans = min(ans, temp);
                }
                dp[i][j] = ans;
            }
        }

        return dp[0][n-1];
    }

public:
    int minScoreTriangulation(vector<int>& values) {
        int n = values.size();
        // return recursive(values, 0, n-1);

        // vector<vector<int>> dp(n, vector<int>(n, -1));
        // return memoization(values, 0, n-1, dp);

        return tabulation(values);
    }
};